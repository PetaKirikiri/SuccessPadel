-- Leaderboard guest metadata + claim flow for retroactive score linking.
...
create or replace function public.claim_padel_player(p_padel_player_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  perform public.link_padel_player_to_profile(p_padel_player_id, v_profile_id);
end;
$body$;

grant execute on function public.claim_padel_player(uuid) to authenticated;
