-- Allow tied game scores (e.g. 1–1 when time runs out); still cap each side at target.
...
begin
  ...
  if v_use_team_scores then
    ...
    if v_unit = 'games' and v_target is not null then
      if v_pts_a > v_target or v_pts_b > v_target then
        raise exception 'Each side must score 0–% games', v_target;
      end if;
