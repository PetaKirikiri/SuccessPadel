-- Competition roster sync: preserve member profile links and padel_player ids per slot.

drop function if exists public.sync_competition_roster_slots(uuid, text[]);
...
grant execute on function public.sync_competition_roster_slots(uuid, jsonb) to anon, authenticated;
