-- Competition game scores: first to N games (winner N, loser < N), not best-of sum.
...
begin
  ...
  if v_use_team_scores then
    ...
    if v_unit = 'games' and v_target is not null then
      if greatest(v_pts_a, v_pts_b) <> v_target or least(v_pts_a, v_pts_b) >= v_target then
        raise exception 'Score must be first to % games', v_target;
      end if;
