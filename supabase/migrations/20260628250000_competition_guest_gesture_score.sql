-- Spectator / guest phones can gesture-score live competition courts without sign-in.
...
begin
  ...
  if p_friendly_session_id is not null then
    ...
    if v_session.visibility <> 'public' and auth.uid() is null then
    ...
    if not exists (
      ...
      raise exception 'Competition not open for scoring';
    end if;
  ...
  if not v_admin and p_friendly_session_id is null and p_competition_id is null then
