-- Rank a proposed roster by positional finishes in completed competitions.
-- A finish earns (field size - finishing position + 1) points. Players without
-- history sort after known players, with random ordering on each request.
create or replace function public.auto_rank_competition_roster(p_players jsonb)
returns jsonb
language sql
volatile
security definer
...
revoke all on function public.auto_rank_competition_roster(jsonb) from public;
grant execute on function public.auto_rank_competition_roster(jsonb) to authenticated;
