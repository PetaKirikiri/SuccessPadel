...
import { readStoredAuthUserId } from '../../lib/auth/cachedSession'
import { resolveProfileAvatarUrl } from '../../lib/resolveProfileAvatar'
import { useTheme } from '../../providers/ThemeProvider'
import { LanguagePicker } from '../NavBar/LanguagePicker'
import { LineSignInModal } from '../Modal/LineSignInModal'
import { IconProfile } from '../NavBar/ShellTabIcons'
...
export function ProfileChip({ returnTo, className = '', navItem = false }: Props) {
  ...
  const { profile, user, loading, restoreSession } = useAuth()
  ...
  useEffect(() => {
    ...
    if (!readStoredAuthUserId()) return
    void restoreSession()
  }, [user, profile, loading, restoreSession])

  const isSignedIn = Boolean(user ?? profile ?? readStoredAuthUserId())
  ...
  const goProfile = () => {
    ...
    if (!uid) return
    setMenuOpen(false)
    navigate(playerProfilePath({ id: uid, displayName: profile?.display_name }))
  }
