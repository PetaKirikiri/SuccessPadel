...
function MemberSlotNameInput({
  ...
  return (
    <input
      ...
      className={inputClassName ?? 'brand-input h-9 min-w-0 flex-1 py-1 text-sm'}
      autoComplete="name"
    />
  )
}
          ...
          >
            ...
            {linkAvatarsToProfile ? (
              <PlayerAvatarLink
                ...
                imgClassName={
                  inviteChipLayout
                    ? 'invite-roster-chip__avatar shrink-0 rounded-full object-cover ring-1 ring-brand-border/80'
                    : 'h-9 w-9 shrink-0 rounded-full object-cover ring-1 ring-brand-border/80'
                }
              />
            ) : (
