...
function LeaderboardShareCard({
  ...
  return (
    <div
      ref={cardRef}
      className="leaderboard-share-card w-[640px] overflow-hidden rounded-3xl border border-[#dddcd0] bg-[#fdfaf5] text-[#3a3a3a] shadow-xl"
      style={{ fontFamily: 'Roboto, ui-sans-serif, system-ui, sans-serif' }}
    >
      <div className="bg-[#602d24] px-6 py-5 text-white">
      ...
      <div className="px-6 pb-5 pt-4">
        <p
          className="mb-3 text-sm font-semibold uppercase tracking-wide text-[#7a7a7a]"
          style={{ fontFamily: 'Roboto Slab, ui-serif, Georgia, serif' }}
        >
          {t('leaderboard.standings')}
        </p>

        <div className="grid grid-cols-[2rem_2.5rem_minmax(0,1fr)_4.5rem_3rem] items-center gap-x-2 border-b border-[#dddcd0] pb-2 text-[10px] font-semibold uppercase tracking-wide text-[#7a7a7a]">
        ...
        <ol className="m-0 list-none p-0">
          {rows.map((row) => (
            <li
              ...
              className={`grid grid-cols-[2rem_2.5rem_minmax(0,1fr)_4.5rem_3rem] items-center gap-x-2 border-b border-[#dddcd0]/80 py-2.5 last:border-0 ${
                row.rank <= 3 ? 'bg-[#fff3da]/70' : ''
              }`}
            >
              <span
                className={`text-center text-sm font-bold ${
                  row.rank <= 3 ? 'text-[#b85a1e]' : 'text-[#7a7a7a]'
                }`}
                style={{ fontFamily: 'Roboto Slab, ui-serif, Georgia, serif' }}
              >
                {row.rank}
              ...
              {row.avatarUrl ? (
                <img
                  src={row.avatarUrl}
                  alt=""
                  className="h-10 w-10 rounded-full object-cover ring-1 ring-[#dddcd0]"
                />
              ) : (
                <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff3da] text-sm font-semibold text-[#602d24] ring-1 ring-[#dddcd0]">
                  {playerInitial(row.name)}
                </span>
              )}
              <span className="truncate text-base font-medium text-[#3a3a3a]">{row.name}</span>
              ...
              <span
                className="text-right text-lg font-bold tabular-nums text-[#b85a1e]"
                style={{ fontFamily: 'Roboto Slab, ui-serif, Georgia, serif' }}
              >
                {row.points}
              </span>
        ...
        <p className="mt-4 text-center text-[11px] font-medium text-[#7a7a7a]">{site}</p>
      </div>
    </div>
  )
}
