...
import { IconJoin } from '../../shared/Button/ButtonIcons'
...
export function GameCardPlaySession() {
    ...
    () =>
      id
        ? previewGames.flatMap((game) =>
            game.matches.map((match) => friendlyCourtSetupKey(id, game.gameNumber, match.courtLabel)),
          )
        : [],
    [id, previewGames],
  )
  ...
  const standingsLogs = useMemo(
    ...
    [matchLogs, organizedConfig, now],
  )

  const standings = useMemo(
    () => computeFriendlySessionStandings(standingsLogs, scoreUnit, sessionRoster),
    [standingsLogs, scoreUnit, sessionRoster],
  )
  ...
  const achievements = useMemo(
    () =>
      sessionStarted
        ...
        : null,
    [sessionStarted, standingsLogs, scoreUnit, sessionRoster, enrichedStandings],
  )
  ...
  const handleScoresSaved = useCallback(async () => {
    await Promise.all([refreshLiveScores(), refresh()])
  }, [refresh, refreshLiveScores])
